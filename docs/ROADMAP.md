...
| --- | ------------------------------------------------------- | ---------- | --------------- | -------- | ------------------------------------------- |
| 1   | **Orders-v2:** Triage вхідних + Timeline-вʼю + bulk-дії | 02 Orders  | ✅ orders-v2    | **П1**   | board+table готові; решта — ні.             |
| 2   | **Board** (InternalTask kanban, фікс-колонки)           | 02/board   | ✅ InternalTask | **П1**   | окремого екрана дошки задач нема.           |
| 3   | **Margin:** per-executor розбивка + тумблер `zeroCost`  | 22 Finance | ✅ `byExecutor` | П2       | margin «за проєктами» є; per-executor — ні. |
| 4   | **Order-chat:** фільтр «Усі / Тільки внутрішні»         | 03 Chat    | ✅              | П2       | дрібний conformance-RISK у ChatTab.         |
| 5   | **Юр-онбординг компанії** (форма реквізитів)            | 06 Docs    | ✅ requisites   | П2       | API GET/PATCH готовий; решта 06 = S6+.      |
