...
- **Картка клієнта 360°** `/clients/:id` — таби Огляд(стати+замовлення+«потребує уваги») · Люди
  (ростер + owner: **запросити** email-інвайтом / роль / видалити) · Проєкти ·
  Фінанси(лояльність+override / маржа YTD) · Документи(PDF) · Реквізити(read + agency
  edit-on-behalf). Manager бачить лише Огляд. _Тест: у клієнта з ≥1 користувачем — «Запросити
  учасника» → лист-інвайт на портал; прийняти інвайт → зʼявляється в ростері як «учасник»._
...
4. **Reset-password on-behalf** (member-mgmt write — email-флоу). ✅ _invite client member — зроблено_ (POST `/workspace/clients/:id/members/invite`, owner-only, reuse portal-accept).
