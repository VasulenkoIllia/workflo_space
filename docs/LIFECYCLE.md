...
## 8. Agency (tenant) lifecycle — SaaS (audit 2026-06)

> Раніше LIFECYCLE покривав лише Profile/Company/Order. Для white-label SaaS потрібен
> tenant-рівень. Поля вже є на `Agency` (`subscriptionStatus`, `suspendedAt`, F1);
> флоу — Enablement (E2/E6), але семантику фіксуємо тут.

### Subscription states (`Agency.subscriptionStatus`)

- `trialing` → `active` → `past_due` → `suspended`/`canceled`.
- `trialing`: новий тенант (signup, E1), `trialEndsAt` виставлено.
- `active`: підписка оплачена (E2).
- `past_due`: платіж не пройшов → dunning; деградація до **read-only** (write-gate
  `assertAgencyActive()` — seam уже закладено no-op'ом, Block 1).
- `suspended`: тривала несплата/порушення → доступ заблоковано, дані збережено.
- `canceled`: тенант пішов → grace-період, далі export + purge.

### Suspend (read-only)

`assertAgencyActive(agencyId)` на write-сайтах кидає `agency_suspended` коли
status ∈ {past_due, suspended, canceled}. Reads лишаються (клієнт бачить, але не пише).
Активувати = одна функція (seam уже протягнуто), без правки кожного хендлера.

### Export (GDPR / off-boarding)

Tenant-export ітерує той самий список таблиць, що RLS-міграція (column- + parent-scoped),
з `wf_in_tenant`-скоупом для `agencyId` → JSON/CSV bundle. Має існувати ДО обіцянок у
privacy-policy (`docs/LEGAL/`). Скелет: один прохід по tenant-таблицях під bypass-context
з явним `WHERE agencyId = :id`.

### Delete / purge

Після canceled + grace: видалити tenant-рядки (FK-каскади де є; інакше — впорядкований
прохід) + блоби storage (`agencies/<agencyId>/...` prefix) + Agency. Незворотно; audit-лог
факту видалення (без даних). `onDelete: Restrict` на фінансових таблицях навмисний —
purge має бути усвідомленим впорядкованим кроком, не випадковим каскадом.
