...
| P-7  | Погодження оцінки (опційне, requiresApproval) + аванс-гейт (опційний) + договір-гейт блок документів (П3). **+ `hourly_prepaid` cycle-двигун (з P-2):** аванс при старті (гейт 02-В) + звірка факту проти авансу при закритті циклу. **🔄 Зроблено (вирішене):** (1) **контракт-гейт П3** ✅ — `generateRecurringCharges` виключає гейтнуті (`NOT {contractRequired,!contractDocumentId}`, nextCycleAt не просувається→catch-up; `GenerateResult.gated`); `closeProjectCycle` 409; `contractDocumentId` settable+валідація (Document type=contract, same company); 4 integration. (2) **internal-task ендпоінт** ✅ (запит власника) — `POST /workspace/orders`: команда створює завдання для обраного клієнта (`internal_task`, `zeroBilled` обирає команда; клієнт бачить у порталі; без погодження); 4 тести. **⏸️ ВІДКЛАДЕНО (грошові owner-форки):** аванс-гейт 02-В (база авансу, джерело трекінгу), `hourly_prepaid` cycle (естімейт авансу з EstimateLine?, представлення кредиту при переплаті), approval-стейт-машина 02-А (Order.approvalStatus, rejection-reason). Потрібні рішення власника                                                                                                                                                                                                                                                       | 02-А, 02-В    | 🔄                           |
