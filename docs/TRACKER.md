...
| P-1  | **3a ✅** `Project`+`ProjectExecutorRate`+enums+міграція RLS (`20260617_p1_projects`), `Order.projectId/zeroBilled`, Project↔LegalEntity, CRUD `/workspace/projects` (гейт `billing`; per-billingModel `contractRequired` §5); тести 8 unit + 5 integration (RLS/unique/каскад). **3b ⬜** повний дроп `CompanyService` (П6) + re-point `ServiceCharge` (`companyServiceId`→`projectId`) + recurring→cycle-engine | 05-ПРОЕКТИ    | 🔄 3a✅ / 3b⬜ |
