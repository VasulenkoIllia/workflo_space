...
| S5-11 | Portal — /billing /wallet /referrals /loyalty. **🔄 /billing ✅** (P-11 фінанси клієнта: summary + рахунки з 3 числами + on_actuals approval-модалка з counter-offer + платежі + реквізити; `a5a3475`, **верифіковано в браузері**). /wallet /loyalty /referrals — ⬜                                                                                                                                                                                                                                                                                                            | [05/25/09/10] | 🔄     |
| S5-12 | Workspace — /billing /payouts /services /team /finance /admin-wallet. **🔄 /billing ✅** (P-11 командна сторона: overview + черга «на погодженні» = internal release + counter-offer + топ-боржники; owner-only, manager finance-blocked; `ee684ba`, **верифіковано в браузері**). /payouts /services /finance /admin-wallet — ⬜. **+ /projects ✅** (фін-проєкт config з UI: модель/ставка/цикл/`approvalMode`/`invoiceApprover`; `GET /workspace/companies` пікер; `25f1fca`, **верифіковано наживо** — створено on_actuals-проєкт через форму). Self-service петля замкнена. | [05/12/22/25] | 🔄     |
