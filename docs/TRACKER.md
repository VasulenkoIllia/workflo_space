...
| P-1  | `Project` + `ProjectExecutorRate` міграція + **заміна `CompanyService`** (refactor, без даних-міграції — П6); видалити legacy-гілку recurringCharges                      | 05-ПРОЕКТИ    | ⬜     |
| P-1б | **Передумова: `LegalEntity` (20-Д)** — спека ✅ [`LEGAL_ENTITY_SPEC.md`](LEGAL_ENTITY_SPEC.md); модель + дефолтна юр-особа в `provisionAgency()` + CRUD (до/разом із P-3) | 20-Д          | ⬜     |
| P-2  | Білінг-цикли (N-число / щотижня / вручну) → cycle-двигун per-проєкт; 3 моделі (абон-аванс/погодинка аванс·факт) + zero-billed задачі в собівартість                       | 06-В          | ⬜     |
| P-3  | Юр-особи агенції (профілі реквізитів) + юр-реквізити клієнта + email для документів                                                                                       | 20-Д, 06-Б    | ⬜     |
| P-4  | Payment terms (строк оплати, налаштовується) → dueDate (основа dunning)                                                                                                   | 05-Г          | ⬜     |
| P-5  | Компенсаційні моделі (ставка / ставка+години / години) + `hourlyRate` + триярус собівартості (zeroCost) + снапшот ставок на TimeLog (П5/П7)                               | 12, 22-нюанси | ⬜     |
| P-6  | Кошторис `EstimateLine` + per-client доступність послуг каталогу                                                                                                          | 02-Б          | ⬜     |
| P-7  | Погодження оцінки (опційне, requiresApproval) + аванс-гейт (опційний) + договір-гейт блок документів (П3)                                                                 | 02-А, 02-В    | ⬜     |
| P-8  | EUR як валюта + валюта бонусів у налаштуваннях (USDT = метод оплати, не валюта)                                                                                           | 05-Е, 09      | ⬜     |
| P-9  | Маржа-движок v1 (accrual + «оплачено N%» П2; собівартість/маржа по проєктах/клієнтах) + реферал-бонус працівника від чистого доходу по клієнту (П4)                       | 22-Д, 12      | ⬜     |
| P-10 | Разові знижки + VAT-поля                                                                                                                                                  | 05-З, 05-Ж    | ⬜     |
...
| ID     | Задача                                                                                                                           | Коли                  |
| ------ | -------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| MOD-1  | **ModuleRegistry** у `@workflo/types`: `{key,name,dependsOn[],appSurface,core}` + граф залежностей (один канон)                  | до/на старті S5.6     |
| MOD-2  | **Дисципліна гейту:** кожен НОВИЙ модуль (S5.6+) гейтить роути+навігацію через `featureEnabled(agencyId,moduleKey)` (seam уже є) | наскрізно з S5.6      |
| MOD-3  | **`BillingPlan.modules`** (через `features Json`) + валідатор «увімкнув модуль → увімкни залежності за графом»                   | до фронтенд-проходу   |
| MOD-4  | Ролі owner/manager/executor у `can()` (20-А) + view-as owner→клієнт (20-Б); платформна імперсонація (SA-1) — SaaS Phase 1        | до фронтенд-проходу   |
