...
| **7 хабів**                  | `WorkspaceBillingHub`, `WsLandingHub`, `ReportsHub`, `AdminSettingsHub`, `AdminHub`, `PortalSettingsHub`, `FinanceHub` — десятки екранів згорнуто під 1 пункт з під-табами (`workspace-hubs.jsx` `makeHub()`) | 🟡 **немає задачі в TRACKER**      |
...
| 14  | **Рольова модель + IA (hubs)**           | Словник ролей `superadmin/lead` → **owner/manager/executor** (узгоджено з `can()` / ADR-008). Нав рольова (`ia-roles.css` · `navVisibleForRole()`). **7 хабів** (`workspace-hubs.jsx`) → потребують окремої фронт-задачі (немає в TRACKER). **Рішення власника:** clients-CRUD + Document-route **НЕ** підтягуємо наперед — за планом (S6–S13).                                        |
