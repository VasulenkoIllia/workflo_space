...
> Статус: **План (авторитетний)** · Створено: 1 червня 2026 · Оновлено: 12 червня 2026 (ADR-008)
> Базується на: [`adr/004-multi-tenancy.md`](adr/004-multi-tenancy.md) + **[`adr/008-saas-module-packaging.md`](adr/008-saas-module-packaging.md)** (пакування модулів у тарифи + ролі/доступ — рішення власника 12.06). Цей документ — ЯК увімкнути продаж платформи іншим агенціям.
> Принцип: **фундамент закладаємо зараз (S0–S2, дешево), увімкнення SaaS робимо в самому кінці (Phase 1), без переписування.**
>
> **Модель пакування (ADR-008, канон):** entitlement-gated моноліт — модулі вмикаються фіч-флагами per-тенант за графом залежностей; продаються наперед визначеними тарифами (Starter/Pro/Business) + add-on'ами, НЕ довільними тумблерами. Foundation-таски — TRACKER §«SaaS-модульність» (MOD-1…4). Лендінг/портал — per-план флаги; Workspace — завжди on.
