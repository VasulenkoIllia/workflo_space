...
### 12-КОМПЕНСАЦІЇ ⭐ (вимога власника): формати оплати працівника

> **Уточнення (12.06, питання власника):** модель НЕ прив'язана до годин. Комісія `%` —
> **ОРТОГОНАЛЬНА опція поверх будь-якої базової моделі**, не окрема модель. Поточна
> продакшн-модель = `monthlySalary + commissionPercent` (`hourlyEarned=0`) — тобто
> «ставка» і «ставка + %» вже працюють; години — майбутнє розширення.

**3 базові моделі × комісія on/off = 6 комбінацій** (`CompensationModel` базова частина):

| Модель               | Складові                                      | Стан                               |
| -------------------- | --------------------------------------------- | ---------------------------------- |
| Ставка               | `monthlySalary`                               | ✅ є                               |
| Ставка + %           | `monthlySalary` + `commissionPercent`×виручка | ✅ **поточна продакшн-модель**     |
| Ставка + години (+%) | + `hourlyRate`×год                            | потребує `ExecutorRate.hourlyRate` |
| Тільки години (+%)   | `hourlyRate`×год                              | потребує `ExecutorRate.hourlyRate` |

`total = baseSalary + hourlyEarned + commissionAmount` (payout.ts) — уже така структура;
бракує лише `hourlyRate`-колонки для моделей з годинами. Потребує: `ExecutorRate.hourlyRate`

- перемикач базової моделі + комісія% як окреме поле (вже є). База `commissionPercent` —
  % від виручки виконавця (revenue), НЕ від годин.

### 12-ПЛАН-ФАКТ (питання власника 12.06): екран план/факт годин

Окремо від оплати — **облік завантаженості**: заплановано N робочих годин, виконано M.
Дані переважно ВЖЕ є: «план» = `capacity` (12-Б, тижнева норма) / `estimatedHours` (на
замовлення) / `includedHoursCap` (на абон-проєкт); «факт» = `TimeLog`. Це **звітний екран**,
не нова модель. **Рішення власника (12.06):** джерело плану = **норма capacity × тижні** (G-2).

**Рівні (G-3):**

| Рівень                   | План                                                                     | Факт                  |
| ------------------------ | ------------------------------------------------------------------------ | --------------------- |
| **Команда (агрегат)** ⭐ | Σ(capacity × тижні) по ВСІХ виконавцях — масштабується з кількістю людей | Σ(TimeLog) по команді |
| Людина / місяць          | capacity-норма людини × тижні                                            | TimeLog людини        |
| Проєкт                   | `estimatedHours` / абон-ліміт                                            | TimeLog проєкту       |
| Замовлення               | `estimatedHours`                                                         | TimeLog замовлення    |

Командний рівень (доповнення власника 12.06): загальна потужність команди vs відпрацьоване —
ключова цифра owner-а для планування найму/завантаження. Рознести у 12-Б/19 + ТЗ дизайнеру.
