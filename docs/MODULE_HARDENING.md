...
| **Gate-2 — дизайн застиг**   | Усі екрани модуля = `РЕЮЗ`                                | У [`DESIGN_SPEC_FULL.md`](DESIGN_SPEC_FULL.md) §модуля жодного `РОЗШИР`/`НОВЕ`; блок «Що перевірити» порожній; є JSX-артборд у `design/project/*.jsx`; намальовані стани empty/loading/error/permissions/mobile. |
