...
>
> ✅ **UPDATE 2026-07-05 (пізніше того ж дня) — 17-Д типізовані шаблони збудовано.** Каталог `VAULT_RESOURCE_TYPES` (crm/server/hosting/api/db/other) + `VAULT_FIELD_KINDS` (url/login/email/note — відкриті; password/api_key/token/secret_w — 🔒) у `@workflo/types` — **єдина точка підміни**, коли власник затвердить фінальний перелік (БД не мігрувати: ключі лишаться ідентифікаторами). Модель: `resourceType` + `publicFields Json` (відкриті поля plain), **усі секретні поля картки шифруються одним envelope-пейлоадом** (JSON у наявних ciphertext-колонках; legacy-рядки без resourceType працюють як раніше). Create приймає обидва шейпи (typed вимагає ≥1 🔒-поля); reveal typed-картки віддає `secretFields[]` (одна audit-подія + один тік throttle на картку). UI спільний в `@workflo/app-core` (`VaultTypedFieldsEditor` + `VaultTypedCardFields`): білдер полів у add-формах обох апок, картка з per-field «копіювати», секретні — маскою до reveal. +6 тестів. **Лишок модуля:** TOTP-модал з мокапа, executor-share, ротація-нагадування, inline-edit.

> ✅ **UPDATE 2026-07-05 (третій зріз дня) — 17-SHARE executor-share збудовано** (модель доступу — рішення власника з вікторини 05.07: **гібрид**, **безстроково до відкликання**, виконавцю — свій розділ «Секрети»). `CredentialShare` (міграція `20260705_credential_shares`, RLS): рівно одна ціль — `credentialId` (точковий) або `companyId` (**всі секрети клієнта, вкл. майбутні**); `expiresAt` лишений nullable під авто-expiry спеки (гейт його перевіряє, UI не видає). Роути `/workspace/vault/shares` GET/POST/DELETE (owner-only; одержувач мусить бути виконавцем агенції — 400; дубль активного → 409). Read-шлях: `services/vaultShareAccess.ts` — owner все · manager ніщо (канон) · executor union своїх активних share; list підрізається скоупом (без share — порожньо, не 403), reveal поза скоупом → 403, step-up відкрито виконавцям. UI: блок «Доступ виконавців» (client-level тумблери) + «доступи» на рядку секрету (точкова модалка; покриті client-share показані замком) + рядок «доступ у команди: імена» (канон дизайну); виконавець — nav «Секрети» → `/vault` read-only (без журналу/CRUD, клієнт plain-текстом). +14 тестів. Вживу: grant → executor бачить/розкриває через свій пароль → revoke → порожньо. **Лишок модуля:** ротація-нагадування, TOTP-модал, inline-edit.
