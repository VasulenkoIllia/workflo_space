...
> **UPDATE 05.07.2026 — секція «Чати» (workspace-хаб) збудована.** Рішення власника:
> групування **клієнт → замовлення**, повний v1 «список + чат + unread».
>
> - **Бек:** `GET /workspace/conversations` (команда) — всі замовлення агенції з
>   чат-активністю: прев'ю останнього повідомлення (140 сим., автор + team/client +
>   🔒internal), **unread** одним raw-запитом поверх `OrderChatRead` (коментарі не від
>   мене, новіші за мій lastReadAt), ефективний відповідальний (chatOwnerId ?? assignee,
>   з іменем), mute/archive стан юзера, прапорець `mine`. Кап 300 тредів, сортування за
>   свіжістю. Фільтри рахує фронт із прапорців.
> - **UI:** `/chats` (workspace, всі внутрішні ролі) — master-detail: зліва треди,
>   згруповані по клієнту, з unread-бейджами/прев'ю/відповідальним; праворуч спільний
>   `<ChatTab>` (повний чат з mute + відповідальним) + кнопка архів/повернути. Фільтри:
>   всі · **мої** (18-В фільтр «мої треди» — закрито) · **без відповіді** (останній автор —
>   клієнт, замовлення активне) · архів. SSE-стрім обраного треда; список авто-оновлюється
>   кожні 30с; відкриття треда маркує прочитаним → бейдж гасне.
> - ~~Лишається з проходу власника: 18-Б snooze.~~ ✅ **18-Б snooze збудовано (06.07)** —
>   модуль 18 ПОВНИЙ. `ConversationState.snoozedUntil` (міграція `20260706_snooze`);
>   `PUT /orders/:id/conversation {snoozeUntil: ISO|null}`; поки час у майбутньому —
>   тред схований з основних фільтрів хабу (всі/мої/без відповіді) і живе у фільтрі
>   **«відкладені»** («⏰ до HH:MM»); після настання часу — повертається у «всі» з
>   ⏰-маркером («повернути непрочитаною» = повернути в поле уваги), відкриття треда
>   знімає маркер. UI: селект «⏰ відкласти…» (1 год / 4 год / завтра 9:00) у хедері
>   чату хабу. Workspace-only (командне поняття; портального snooze немає свідомо).

> **UPDATE 05.07.2026 (пізніше) — 18-А portal-inbox збудовано.** Той самий підхід у
> портальному соусі:
>
> - **Бек:** `GET /portal/conversations` — КЛІЄНТСЬКИЙ скоуп: лише замовлення моїх компаній
>   і **лише публічні повідомлення** — внутрішні нотатки команди для порталу не існують ані
>   в прев'ю, ані в unread (окремий `isInternal = false` фільтр у raw-запиті; перевірено
>   тестом і вживу — internal-нотатка власника не зрушила ні прев'ю, ні лічильник).
>   Без відповідального/`mine` (командні поняття). Акаунт без компанії → грейсфул `[]`.
> - **UI:** `/chats` у порталі (нав «Чати» після «Замовлення») — master-detail зі спільним
>   портальним `<ChatTab>` (клієнтські capabilities), фільтри «всі / архів», архів/повернути,
>   unread-бейджі, SSE обраного треда. **Обидва хаби (portal + workspace) отримали
>   narrow-режим:** ≤720px master-detail стає «список ⇄ чат» з кнопкою «← список».
