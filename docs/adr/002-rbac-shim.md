...
export function can(user: AuthContext, action: Action, resource: ResourceContext = {}): boolean {
  ...
  if (action.startsWith('order.') || action.startsWith('invoice.') || action.startsWith('payment.')) {
...
// у service layer
export async function transitionOrderStatus(actor: AuthContext, orderId: string, next: OrderInternalStatus) {
