...
-- 1. Backfill: for every company that has an owner_id, ensure a matching
...
-- 2. Sanity check: every company must have exactly one owner.
DO $$
DECLARE
  bad_count INT;
BEGIN
...
-- 3. Drop the old constraint + column.
...
-- 4. Add partial unique index: at most one owner per company.
