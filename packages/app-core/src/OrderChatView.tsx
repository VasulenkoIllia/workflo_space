...
/**
 * Shared order-chat UI (AR-42 decomp C). The workspace (team) and portal (client)
 * ChatTabs were ~93% identical; this component is the single source, parameterised
 * by a small capabilities object. Each app keeps a thin wrapper that calls its own
 * `useAuth` and passes `currentUserId` + `capabilities` down. File upload stays
 * app-specific: the wrapper injects an `useUploadFile`-backed uploader.
 */
...
/** Capabilities that differ between the team (workspace) and client (portal) chat. */
export interface OrderChatCapabilities {
  /** Може закріплювати/відкріплювати повідомлення (03-Г pin-контроли). */
  canPin: boolean
  /** Показує FilterBtn «Усі / внутрішні» + маркер 🔒 внутрішніх нотаток. */
  canFilterInternal: boolean
  /** Має тумблер «internal» на композері (постить із isInternal). */
  canPostInternal: boolean
  /** Може редагувати/видаляти будь-чиї повідомлення (owner), не лише власні. */
  canEditAnyone: boolean
}

/** Аплоадер файлу — app-specific (useUploadFile живе в кожному lib/orderDetail). */
export interface OrderChatUploader {
  mutateAsync: (file: File) => Promise<{ file: CommentAttachment }>
  isPending: boolean
}

export interface OrderChatProps {
  orderId: string
  /** profile-id поточного користувача (для «mine»-перевірок і фільтру @-кандидатів). */
  currentUserId: string
  capabilities: OrderChatCapabilities
  upload: OrderChatUploader
}

export function OrderChat({ orderId, currentUserId, capabilities, upload }: OrderChatProps) {
  const { canPin, canFilterInternal, canPostInternal, canEditAnyone } = capabilities
  ...
  const send = () => {
    ...
    post.mutate(
      {
        content,
        ...(canPostInternal ? { isInternal: internal } : {}),
  ...
  const baseShown =
    canFilterInternal && onlyInternal ? comments.filter((c) => c.isInternal) : comments
          ...
          return (
            ...
            >
              ...
              >
                ...
                {canFilterInternal && c.isInternal && ' 🔒'}
              </span>
