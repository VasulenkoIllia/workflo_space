...
function makePrisma(prefs: Array<{ channel: string }>): ResolverPrisma {
  return {
...
describe('resolveTargetChannels', () => {
  it('returns enabled channels for a non-critical event', async () => {
    const prisma = makePrisma([{ channel: 'email' }, { channel: 'telegram' }, { channel: 'in_app' }])
  ...
  it('returns empty when no prefs and event is non-critical', async () => {
    const prisma = makePrisma([])
    const result = await resolveTargetChannels(prisma, 'settings-1', 'orders.status_changed')
    expect(result).toEqual([])
  })

  it('returns email-only when critical event and zero prefs', async () => {
    const prisma = makePrisma([])
  ...
  it('queries Prisma with the correct category from EVENT_TO_CATEGORY', async () => {
    ...
    expect(findMany).toHaveBeenCalledWith({
      where: { settingsId: 'settings-1', category: 'chat', enabled: true },
      select: { channel: true },
    })
  })
  ...
  it('returns channels in stable order: email, telegram, in_app', async () => {
    const prisma = makePrisma([{ channel: 'in_app' }, { channel: 'telegram' }, { channel: 'email' }])
    const result = await resolveTargetChannels(prisma, 'settings-1', 'orders.status_changed')
    expect(result).toEqual([
      NotificationChannel.EMAIL,
      NotificationChannel.TELEGRAM,
